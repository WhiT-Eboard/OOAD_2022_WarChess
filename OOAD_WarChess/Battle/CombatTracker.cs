...
public class CombatTracker
{
    ...
    public string CombatLogToString(Tuple<string, string, string, int, string, LogType> log)
    {
    ...
    {
        _combatLogList.Add(Tuple.Create(pawn.Name, modifier.Name, modifier.Giver.Name, modifier.Duration, "", LogType.ModifierGain));
        if (IsDebug)
        {
            Console.WriteLine(CombatLogToString(_combatLogList.Last()));
        }
